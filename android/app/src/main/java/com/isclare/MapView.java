...
public class MapView extends TMapView {
    ...
    public void addMarker(ReadableArray orderList) {
        Log.d("view", "called from view(coord)" + orderList.getMap(0).getDouble("startLat") + ", "
                + orderList.getMap(0).getDouble("startLong"));
        Log.d("view2", "called from view(name)" + orderList.getMap(0).getString("startAddr"));
        for (int i = 0; i < orderList.size(); i++) {
            TMapPoint tMapPointS = new TMapPoint(orderList.getMap(i).getDouble("startLong"),
                    orderList.getMap(i).getDouble("startLat"));
            ...
            TMapPoint tMapPointE = new TMapPoint(orderList.getMap(i).getDouble("endLong"),
                    orderList.getMap(i).getDouble("endLat"));
